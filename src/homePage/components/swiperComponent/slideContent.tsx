...
export function PcSlideContent({ image, link, title, description, button, name, }: SlideProps) {
    ...
    return (
        <div className="relative text-white h-[620px] rounded-4xl overflow-hidden">
            <a href={link} target="_blank" className="w-full h-full block">
                <div className="absolute z-10 top-50 w-[45%] left-20">
                    {loading ? (
                ...
                {loading ? (<Skeleton className="w-full rounded-xl" />) : (
                    <img loading="eager" src={image} alt={name} className="w-full h-[90%] object-cover rounded-xl" />
                )}
            </a>
        </div>
    );
